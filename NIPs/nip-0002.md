...
``web+nem://transaction?data=<data>&chainId=<chainId>&endpoint=<endpoint>``
...
| endpoint | (optional) The preferred server to announce the transaction. The application could have another strategy to select the endpoint, omitting this field. | http://localhost:3000 |
