...
`*` Build a separate service to share the default download URLs with randomized values for ``networkGenerationHashSeed``, ``mosaicIds``, ``distribution`` list, and ``sinkAddresses``. As a future enhancement, provide a multi-step form to define the nemesis block properties file step by step.
