...
| **BatchTransferOwnership** | Batch Transfer tokens to new owners (Also used for issuance) |
...
Commands will be grouped in **namespaces** such that each Token Standard can implement commands with its own feature set integration practises.

### 2. New standards could be dealt with in the form of git submodules
...
### 3. Disposability and re-use of securities contracts
...
### 4. Network agnostic token standards
...
:note: A first draft implementation of `symbol-token-standards` will assume usage of Testnet and uses `symbol-hd-wallets` for key derivations.

### 5. Deterministic accounts generation

The draft implementation of NIP 13 includes the usage of `symbol-hd-wallets` to derive deterministic **operator accounts** and **partition accounts** that are later converted to multi-signature accounts.
...
| Implement lock/unlock token command | @evias | ![Progress](https://progress-bar.dev/20/) |
| Implement force transfer token command | @evias | ![Progress](https://progress-bar.dev/00/) |
